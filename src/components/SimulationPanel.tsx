...
export function SimulationPanel({ result }: SimulationPanelProps) {
  ...
  return (
    <div className="space-y-3">
      <div className={`flex items-center justify-between p-2 rounded-md border ${isSingleOperation
          ? singleOpSuccess
            ? 'bg-green-50 border-green-300'
            : 'bg-red-50 border-red-300'
          : 'bg-white border-gray-200'
        }`}>
        <h3 className={`text-sm font-medium ${isSingleOperation
            ? singleOpSuccess
              ? 'text-green-700'
              : 'text-red-700'
            : 'text-gray-900'
        ...
        <span className={`px-2 py-0.5 rounded text-xs font-medium ${result.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
          }`}>
              ...
              <div className="p-4 bg-amber-50 rounded-lg border border-amber-200">
                ...
                <div className="space-y-2">
                  ...
                  <div className="pt-2 border-t border-amber-300">
                    ...
                    <p className="text-sm font-medium text-amber-700">
                      {(result.enhancedDebugInfo.timing.simulationTime / 60000).toFixed(2)} min
                    </p>
                  </div>
                </div>
              </div>
      ...
      {result.enhancedDebugInfo?.operationBreakdown && (
        <div className={`bg-white rounded-lg border-2 shadow-sm ${result.enhancedDebugInfo.operationBreakdown.length === 1
            ? result.enhancedDebugInfo.operationBreakdown[0].success
              ? 'border-green-300'
              : 'border-red-300'
            : 'border-gray-300'
          }`}>
          <div className={`p-4 ${result.enhancedDebugInfo.operationBreakdown.length === 1
              ? result.enhancedDebugInfo.operationBreakdown[0].success
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Activity className={`w-5 h-5 ${result.enhancedDebugInfo.operationBreakdown.length === 1
                    ? result.enhancedDebugInfo.operationBreakdown[0].success
                      ? 'text-green-600'
                      : 'text-red-600'
                    : 'text-blue-600'
                  }`} />
                <h3 className={`text-lg font-semibold ${result.enhancedDebugInfo.operationBreakdown.length === 1
                    ? result.enhancedDebugInfo.operationBreakdown[0].success
                      ? 'text-green-800'
                      : 'text-red-800'
                    : 'text-blue-800'
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${result.enhancedDebugInfo.operationBreakdown.length === 1
                  ? result.enhancedDebugInfo.operationBreakdown[0].success
            ...
            <div key={index} className="px-4 py-3 border-t border-gray-200">
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${op.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
        ...
        <div className="mt-4">
          ...
          <ul className="space-y-2">
            {result.potentialErrors.map((error, index) => (
              <li key={index} className={`text-sm p-2 rounded ${result.success ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-600'
                }`}>
                {error}
              </li>
            ))}
